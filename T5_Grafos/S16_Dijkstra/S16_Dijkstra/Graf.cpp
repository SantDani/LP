...
void Graf::dijkstra(int node, vector<int>& dist)
{
	dist.resize(m_numNodes, DIST_MAX);
	vector<bool> visitat;
	visitat.resize(m_numNodes, false);
	...
	{
		int posVeiAct;
		posVeiAct = minDistance(dist, visitat);

		for (int posVei = 0;  posVei < m_numNodes;  posVei++)
		{
			if (!visitat[posVei] && m_matriuAdj[posVeiAct][posVei] != 0)
			{
				if (dist[posVeiAct] != DIST_MAX && (dist[posVeiAct] + m_matriuAdj[posVeiAct][posVei] < dist[posVei]))
				{
...
int Graf::minDistance(vector<int>& dist, vector<bool>& visitat)
{
	//Inicializar min value
	int min = DIST_MAX;
	int minIndex = -1;
...
void Graf::camiMesCurt(string nodeInicial, string nodeFinal, stack<string>& cami)
{
	vector<string>::iterator itNodoInicial = find(m_nodes.begin(), m_nodes.end(), nodeInicial);
	vector<string>::iterator itNodoFinal = find(m_nodes.begin(), m_nodes.end(), nodeFinal);
	
	if ((itNodoInicial != m_nodes.end() && itNodoFinal != m_nodes.end()))
	{
		int pos1 = distance(m_nodes.begin(), itNodoInicial);
		int pos2 = distance(m_nodes.begin(), itNodoFinal);
...
void Graf::dijstraModificat(int nodeInicial, int nodeFinal, vector<int>& dist, vector<int>& anterior)
{
	dist.resize(m_numNodes, DIST_MAX);
	//vector
}
